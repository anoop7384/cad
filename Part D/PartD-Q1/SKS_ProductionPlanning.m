...
penalty_R3 = 0;
TotalR3Reqd = sum(R3Reqd);
if TotalR3Reqd < AvailRaw3                       % Checking for violation of the raw material 2 constraint
    penalty_R3 = (TotalR3Reqd  - AvailRaw3)^2;   % Determining the penalty due to violation for raw material 2
end

profit = sum(Revenue) - sum(PC);                 % Determining the profit (objective)

f = -profit + 10^15*(penalty_IC + penalty_R1 + penalty_R2 + penalty_R3 + sum(penalty_domain));   % Determining the fitness function value
